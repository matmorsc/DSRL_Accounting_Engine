from __future__ import annotations

from pathlib import Path

import pandas as pd

from src.matching.legacy import match_legacy_payments_to_renewals
from src.posting.batches import build_quickbooks_posting_batches
from src.posting.engine import build_posting_status


def test_stripe_batch_combines_payments_and_fee() -> None:
    gl = pd.DataFrame([
        {
            "account": "Business Checking (0002) - 1",
            "transaction_date": pd.Timestamp("2025-10-14"),
            "transaction_type": "Payment",
            "number": "",
            "name": "Guest A",
            "memo": "",
            "split_account": "Accounts Receivable (A/R)",
            "amount": 650.0,
            "identified_processor": "",
        },
        {
            "account": "Business Checking (0002) - 1",
            "transaction_date": pd.Timestamp("2025-10-14"),
            "transaction_type": "Payment",
            "number": "",
            "name": "Guest B",
            "memo": "",
            "split_account": "Accounts Receivable (A/R)",
            "amount": 2800.0,
            "identified_processor": "",
        },
        {
            "account": "Business Checking (0002) - 1",
            "transaction_date": pd.Timestamp("2025-10-14"),
            "transaction_type": "Expense",
            "number": "",
            "name": "Stripe",
            "memo": "",
            "split_account": "Bank Charges & Fees:Stripe Processing Fees",
            "amount": -101.25,
            "identified_processor": "Stripe",
        },
    ])

    batches = build_quickbooks_posting_batches(gl)

    assert len(batches) == 1
    assert batches.loc[0, "gross_inflows"] == 3450.0
    assert batches.loc[0, "processor_fees"] == -101.25
    assert batches.loc[0, "net_posted_amount"] == 3348.75


def test_combined_same_day_payouts_match_one_batch(
    tmp_path: Path,
) -> None:
    payouts = pd.DataFrame([
        {
            "payout_id": "po1",
            "processor": "Stripe",
            "processor_account": "Legacy",
            "transaction_date": pd.Timestamp("2025-11-12"),
            "payout_amount": 635.35,
            "bank_transaction_id": "b1",
            "bank_transaction_date": pd.Timestamp("2025-11-12"),
            "bank_amount": 635.35,
            "bank_match_status": "Matched",
        },
        {
            "payout_id": "po2",
            "processor": "Stripe",
            "processor_account": "Main",
            "transaction_date": pd.Timestamp("2025-11-12"),
            "payout_amount": 532.25,
            "bank_transaction_id": "b2",
            "bank_transaction_date": pd.Timestamp("2025-11-12"),
            "bank_amount": 532.25,
            "bank_match_status": "Matched",
        },
    ])

    batches = pd.DataFrame([{
        "qb_batch_id": "QB-STRIPE-2025-11-12",
        "processor": "Stripe",
        "batch_date": pd.Timestamp("2025-11-12"),
        "gross_inflows": 1200.73,
        "processor_fees": -33.13,
        "net_posted_amount": 1167.60,
    }])

    overrides = tmp_path / "posting_overrides.csv"
    overrides.write_text(
        "payout_id,bank_transaction_id,posting_status,"
        "quickbooks_reference,notes\n",
        encoding="utf-8",
    )

    result = build_posting_status(
        payout_ledger=payouts,
        quickbooks_gl=pd.DataFrame(),
        quickbooks_batches=batches,
        posting_overrides_path=overrides,
        assume_posted_through="2026-05-15",
    )

    assert set(result["posting_status"]) == {"Already Posted"}
    assert all(
        result["posting_match_method"].str.contains(
            "Combined"
        )
    )


def test_legacy_payment_matches_monthly_renewal() -> None:
    payments = pd.DataFrame([{
        "payment_event_id": "Legacy::txn1",
        "processor_account": "Legacy Cognito",
        "transaction_id": "txn1",
        "transaction_type": "charge",
        "transaction_date": pd.Timestamp("2026-02-19"),
        "gross_amount": 650.0,
    }])

    renewals = pd.DataFrame([{
        "renewal_submission_id": "27",
        "submitted_at": pd.Timestamp("2026-02-19"),
        "tenant_name": "Jessica Adams",
        "unit_site": "Cabin 10",
        "term_start_date": pd.Timestamp("2026-02-01"),
        "total_amount_due": 1300.0,
        "payment_amount": 650.0,
    }])

    result = match_legacy_payments_to_renewals(
        payments,
        renewals,
    )

    assert result.loc[0, "match_status"] == "Exact Amount/Date Match"
    assert result.loc[0, "tenant_name"] == "Jessica Adams"
